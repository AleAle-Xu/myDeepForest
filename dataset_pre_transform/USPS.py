"""
USPS dataset preprocessing (from libsvm).
- libsvm format: already numeric, no missing values
- train: 7291 samples, test: 2007 samples, 256 features, 10 classes (digits 1-10)
...
X_tr, y_tr = load_svmlight_file(os.path.join(raw_dir, "usps", "usps"))
X_t, y_t = load_svmlight_file(os.path.join(raw_dir, "usps", "usps.t"), n_features=X_tr.shape[1])
...
out_path = os.path.join(get_dir_in_root("dataset"), "USPS.csv")
